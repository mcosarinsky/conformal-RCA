...
def get_connected_components(mask):
    ...
    try:
        ...
        np_mask = mask.to(torch.uint8).contiguous().cpu().numpy()  # (N, 1, H, W)
        N = np_mask.shape[0]
        H, W = np_mask.shape[2:]

        labels = np.zeros((N, 1, H, W), dtype=np.int32)
        counts = np.zeros((N, 1, H, W), dtype=np.int32)

        for i in range(N):
            labeled, num = skimage.measure.label(
                np_mask[i, 0], return_num=True, connectivity=2
            )
            labels[i, 0] = labeled
            # compute areas of each component
            for label_id in range(1, num + 1):
                area = np.sum(labeled == label_id)
                counts[i, 0][labeled == label_id] = area

        return torch.from_numpy(labels), torch.from_numpy(counts)
